...
fn main() {
    ...
    if z0vektor[4] == 0 {
        ...
        //17x1
        {
            ...
            //This is the 1x17 so two matches
            for i in LMOD17ARR {
                j = i.into();
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD17ARR[lupval], &LMOD1ARR[rdnval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD17ARR[ldnval], &LMOD1ARR[rupval]
                    );
                    break;
                }
                if chkval < height.try_into().unwrap() {
        ...
        //7x11
        {
            ...
            //This is the 7x11 so two matches
            for i in LMOD7ARR {
                j = i.into();
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD7ARR[lupval], &LMOD11ARR[rdnval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD7ARR[ldnval], &LMOD11ARR[rupval]
                    );
                    break;
                }
                if chkval < height.try_into().unwrap() {
        ...
        //29x13
        {
            ...
            //This is the 29x13 so two matches
            for i in LMOD29ARR {
                j = i.into();
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD29ARR[lupval], &LMOD13ARR[rdnval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD29ARR[ldnval], &LMOD13ARR[rupval]
                    );
                    break;
                }
                if chkval < height.try_into().unwrap() {
        ...
        //19x23
        {
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD19ARR[lupval], &LMOD23ARR[rdnval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
            ...
            loop {
                ...
                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD19ARR[ldnval], &LMOD23ARR[rupval]
                    );
                    break;
                }
                if chkval < height.try_into().unwrap() {
    ...
    if z0vektor[5] == 0 {
        ...
        //7x7
        {
            ...
            //This is the 7x7 fn so can be condensed into one match.
            for i in LMOD7ARR {
                j = i.into();
            ...
            loop {
                ...
                if upval > LMOD7ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD7ARR[upval].clone().into();
                let chkval2: u128 = LMOD7ARR[dnval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD7ARR[dnval], &LMOD7ARR[upval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
        ...
        //13x13
        {
            ...
            //This is the 13x13 fn so can be condensed into one match.
            for i in LMOD13ARR {
                j = i.into();
            ...
            loop {
                ...
                if upval > LMOD13ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD13ARR[upval].clone().into();
                let chkval2: u128 = LMOD13ARR[dnval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD13ARR[dnval], &LMOD13ARR[upval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
        ...
        //17x17
        {
            ...
            //This is the 17x17 fn so can be condensed into one match.
            for i in LMOD17ARR {
                j = i.into();
            ...
            loop {
                ...
                if upval > LMOD17ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD17ARR[upval].clone().into();
                let chkval2: u128 = LMOD17ARR[dnval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD17ARR[dnval], &LMOD17ARR[upval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
        ...
        //23x23
        {
            ...
            //This is the 23x23 fn so can be condensed into one match.
            for i in LMOD23ARR {
                j = i.into();
            ...
            loop {
                ...
                if upval > LMOD23ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD23ARR[upval].clone().into();
                let chkval2: u128 = LMOD23ARR[dnval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD23ARR[dnval], &LMOD23ARR[upval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
        ...
        //1x19
        {
            ...
            //This is the 1x19 so two matches
            for i in LMOD1ARR {
                j = i.into();
            ...
            loop {
                ...
                if lupval > LMOD1ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD1ARR[lupval].clone().into();
                let chkval2: u128 = LMOD19ARR[rdnval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD1ARR[lupval], &LMOD19ARR[rdnval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
            ...
            loop {
                ...
                if rupval > LMOD19ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD1ARR[ldnval].clone().into();
                let chkval2: u128 = LMOD19ARR[rupval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD1ARR[ldnval], &LMOD19ARR[rupval]
                    );
                    break;
                }
                if chkval < height.try_into().unwrap() {
        ...
        //11x29
        {
            ...
            //This is the 11x29 so two matches
            for i in LMOD11ARR {
                j = i.into();
            ...
            loop {
                ...
                if lupval > LMOD11ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD11ARR[lupval].clone().into();
                let chkval2: u128 = LMOD29ARR[rdnval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD11ARR[lupval], &LMOD29ARR[rdnval]
                    );
                    break;
                }
                if chkval > height.try_into().unwrap() {
            ...
            loop {
                ...
                if rupval > LMOD29ARR.len() - 1 {
                    break;
                }

                let chkval1: u128 = LMOD11ARR[ldnval].clone().into();
                let chkval2: u128 = LMOD29ARR[rupval].clone().into();
                let chkval: u128 = chkval1 * chkval2;

                if chkval == height.try_into().unwrap() {
                    println!(
                        "******************\n--result-> {:?} x {:?}\n***************",
                        &LMOD11ARR[ldnval], &LMOD29ARR[rupval]
                    );
                    break;
                }
                if chkval < height.try_into().unwrap() {
